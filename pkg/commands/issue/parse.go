...
// Package issue parses the github or gitlab issue and exports any tags to env.
package issue
...
// ParseCommand fetches and parses an issue and prints out all tags.
...
func (c *ParseCommand) Desc() string {
	return "Export issue tags to env."
}
...
func (c *ParseCommand) Run(ctx context.Context, args []string) error {
	metricswrap.WriteMetric(ctx, "command_issue", 1)
...
// Process handles the main logic for the tagrep issue.
func (c *ParseCommand) Process(ctx context.Context) (merr error) {
	logger := logging.FromContext(ctx)
	logger.DebugContext(ctx, "starting tagrep issue",
		"platform", c.platformConfig.Type)

	body, err := c.platformClient.GetIssueBody(ctx)
	if err != nil {
		merr = errors.Join(merr, fmt.Errorf("failed to get issue body: %w", err))
	}
